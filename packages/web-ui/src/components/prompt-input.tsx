import { Button, IconButton, Select, SelectDialog, Icon } from "@opencode-ai/ui"
import { createEffect, createMemo, Show, For, onMount, onCleanup, createSignal } from "solid-js"
...
export function PromptInput(props: PromptInputProps) {
  ...
  const defaultParts: ContentPart[] = [{ type: "text", content: "", start: 0, end: 0 }]
  ...
  const isEmpty = createMemo(() => {
    if (store.contentParts.length !== 1) return false
    const p = store.contentParts[0]
  ...
  createEffect(() => {
    const currentParts = store.contentParts
  ...
  const addPart = (part: ContentPart) => {
    ...
    const res = store.contentParts.reduce(
      (acc, item) => {
        if (acc.inserted) {
    ...
    setStore("contentParts", res.parts)
    setStore("popoverIsOpen", false)

    queueMicrotask(() => setCursorPosition(editorRef, res.cursorPositionAfter))
  }
  ...
  return (
    <div class="relative size-full max-h-80 flex flex-col gap-3">
      <Show when={store.popoverIsOpen}>
        <div class="absolute inset-x-0 -top-3 -translate-y-full origin-bottom-left max-h-64 min-h-10 overflow-y-auto flex flex-col p-2 pb-0 rounded-2xl border border-border-base bg-surface-raised-stronger-non-alpha shadow-md z-50">
          <For each={flat()}>
            {(i) => (
              <div
                classList={{
