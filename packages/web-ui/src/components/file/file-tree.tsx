...
import { Tooltip, Collapsible } from "@opencode-ai/ui"
...
export default function FileTree(props: {
  ...
  const Node = (p: ParentProps & ComponentProps<"div"> & { node: LocalFile; as?: "div" | "button" }) => (
    <Dynamic
      component={p.as ?? "div"}
      classList={{
        "p-0.5 w-full flex items-center gap-x-2 hover:bg-background-element rounded transition-colors": true,
    ...
    >
      ...
      <span
        classList={{
          "text-12-regular whitespace-nowrap truncate flex-1": true,
          "text-text-muted/40": p.node.ignored,
          "text-text-muted/80": !p.node.ignored,
          "!text-text-strong": local.file.active()?.path === p.node.path,
      ...
      <Show when={local.file.changed(p.node.path)}>
        <span class="mr-1 w-1.5 h-1.5 rounded-full bg-primary/50 shrink-0" />
      </Show>
    </Dynamic>
  )
            ...
            <Switch>
              <Match when={node.type === "directory"}>
                ...
                >
                  <Collapsible.Trigger>
                    <Node node={node}>
                      <Collapsible.Arrow class="text-text-muted/60 ml-1 size-4" />
                      <FileIcon node={node} expanded={local.file.node(node.path).expanded} class="text-text-muted/60" />
                    </Node>
                  </Collapsible.Trigger>
                  <Collapsible.Content>
